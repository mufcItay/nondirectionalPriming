function [signConsistency] = getSignConsistency(RT,label,Nsplits)
...
for i_s = 1:Nsplits
    ...
    group0_sign = sign(median(RT(group==0 & label==1))-median(RT(group==0 & label==0)));
    group1_sign = sign(median(RT(group==1 & label==1))-median(RT(group==1 & label==0)));
