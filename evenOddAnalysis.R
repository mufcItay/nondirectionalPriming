...
uc <- biga[biga$binpercept == 'unaware',] %>%
  group_by(suj,cong,cue) %>%
  mutate(trial = 1:n(),
         trial_oddity = trial%%2) %>%
  select(suj,trial_oddity,cong,cue,rt)

# calculate the orginal effects: first average across trials, then across congreuncy conditions (for each cue X subject)
condsSplit <- uc %>% group_by(suj,cong,cue) %>% summarise(rtMedian = median(rt))
originalEffects <- condsSplit %>% group_by(suj,cue) %>% summarise(effect = diff(rtMedian))

#calculate sign product effects
signProductEffects <- uc %>% 
    group_by(suj,trial_oddity,cong,cue) %>% 
    summarise(rtMedian = median(rt)) %>%
    spread(cong,rtMedian) %>%
    mutate(diff = incong-cong) %>%
    select(suj,trial_oddity,cue,diff)%>%
    spread(trial_oddity,diff,sep='')%>%
    mutate(SPeffect = sign(trial_oddity1)*
             sign(trial_oddity0)*sqrt(abs(trial_oddity0*trial_oddity1))) %>%
    select(suj,cue,SPeffect)
    
merge(originalEffects,
      signProductEffects,
      by=c('suj','cue')) %>%
  ggplot(aes(x=effect,y=SPeffect,color=cue)) +
  geom_point() +
  geom_abline(slope=0,intercept=0) +
  geom_vline(xintercept=0)

ggsave('effectsScatter.png')
